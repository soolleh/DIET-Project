...
function RoutesConfig($stateProvider, $urlRouterProvider,$locationProvider) {
 ...
 // $urlRouterProvider.otherwise('/home');
  ...
  .state('about-us', {
   ...
   .state('downloads', {
    url: '/info',
    templateUrl: 'src/shoppinglist/templates/downloads.template.php',
    data:{
      title:"Downloads"
    }
  })
  ...
  .state('department-Detail', {
    ...
    resolve: {
      item: ['DepartmentService','$stateParams', function (DepartmentService,$stateParams) {
        ...
        .then(function(item){
          return item.data[$stateParams.department_id.toUpperCase()];
        });
      }],
    ...
    .state('about-district', {
    url: '/about-district',
    templateUrl: 'src/shoppinglist/templates/about-district.template.html',
    controller:'aboutDistrictController as districtController',
    data:{
  ...
  .state('schools', {
    url: '/schools',
    templateUrl: 'src/shoppinglist/templates/schools.template.html',
    // controller:'aboutDistrictController as districtController',
    data:{
      title:"Schools"
    }
...
.state('notify', {
    ...
    templateUrl: 'src/shoppinglist/templates/New.Text.Document.html',
       data: {
      title: "notify"
    }
  })
      .state('districtMap', {
    url: '/district-map',
    templateUrl: 'src/shoppinglist/templates/districtmap.template.html',
    // controller:'aboutDistrictController as districtController',
    data:{
      title:"District Map"
    }
    })
    .state('coursesOffered', {
    url: '/courses-offered',
    templateUrl: 'src/shoppinglist/templates/courses-offered.template.html',
    // controller:'aboutDistrictController as districtController',
    data:{
    ...
    .state('8thclass', {
    url: '/classes/8th',
    templateUrl: 'src/shoppinglist/templates/8thclass.template.html',
    // controller:'aboutDistrictController as districtController',
    data:{
      title:"8th Class"
    }
    ...
    .state('8th-eBooks', {
    url: '/classes/8th/eBooks',
    templateUrl: 'src/shoppinglist/templates/8th-eBooks.template.html',
    // controller:'aboutDistrictController as districtController',
    data:{
      title:"8th-Ebooks"
    }
...
function DepartmentService($http) {
  ...
  service.getDepartmentDetail = function () {
    ...
    service.getDepartmentFacultyDetail = function (department_id) {
      var departmentId = department_id.toUpperCase();
      var response =  $http({
        method: "GET",
...
///***************************************************about-district.controller********************************

angular.module('dietApp')
.controller('aboutDistrictController', aboutDistrictController);


aboutDistrictController.$inject = [];
function aboutDistrictController() {
  var districtController = this;
  districtController.openTab = function(evt, cityName){
        var i, x, tablinks;
...
//**********************************************nav controller****************************************************
angular.module('dietApp')
.controller('nabarcontroller', nabarcontroller);


nabarcontroller.$inject = ['$timeout'];
function nabarcontroller($timeout) {
  var navcontrol = this;
  navcontrol.show = true;
  var scroll = window.scrollTo(0,100);
  console.log(scroll);
  navcontrol.hide = function(){
    // $timeout(function () {
    ...
    // }, 1000);
      ...
      // navcontrol.class="navbar-fixed-top";
    
    
    
    
  };
  navcontrol.showNow = function(){
    navcontrol.class=false;
    // navcontrol.show = true;
  };
}
...
function departmentDetailController($stateParams,item,faculty,$filter) {
  ...
  console.log(departmentDetail.items)
  departmentDetail.faculty = faculty.data;
   departmentDetail.department = $stateParams.department_id.toUpperCase();
